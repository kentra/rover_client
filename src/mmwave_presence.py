...
class MMWave:
    ...
    def _send(self, command_data_bytes):
        ...
        packet = (
            COMMAND_HEADER
            + len(command_data_bytes).to_bytes(2, "little")
            + command_data_bytes
            + COMMAND_FOOTER
        )
    ...
    def set_basic_config(self, last_motion_gate, last_static_gate, presence_timeout):
        ...
        if last_motion_gate < 2 or last_motion_gate > LAST_GATE:
            raise ValueError(
                f"Motion gate number {last_motion_gate} out of range 2-{LAST_GATE}"
            )
        if last_static_gate < 2 or last_static_gate > LAST_GATE:
            raise ValueError(
                f"Static gate number {last_static_gate} out of range 2-{LAST_GATE}"
            )
        if presence_timeout < 0 or presence_timeout > MAX_LEGIT_PRESENCE_TIMEOUT:
            raise ValueError(
                f"Presence timeout {presence_timeout} out of range 0-{MAX_LEGIT_PRESENCE_TIMEOUT}"
            )

        data = (
            int(0).to_bytes(2, "little")
            + int(last_motion_gate).to_bytes(4, "little")
            + int(1).to_bytes(2, "little")
            + int(last_static_gate).to_bytes(4, "little")
            + int(2).to_bytes(2, "little")
            + int(presence_timeout).to_bytes(4, "little")
        )
    ...
    def set_gate_sensitivity(self, gate, motion_sensitivity, static_sensitivity):
        ...
        if motion_sensitivity < 0 or motion_sensitivity > 100:
            raise ValueError(
                f"Motion sensitivity {motion_sensitivity} out of range 0-100"
            )
        if static_sensitivity < 0 or static_sensitivity > 100:
            raise ValueError(
                f"Statiction sensitivity {static_sensitivity} out of range 0-100"
            )
        ...
        data = (
            int(0).to_bytes(2, "little")
            + int(gate).to_bytes(4, "little")
            + int(1).to_bytes(2, "little")
            + int(motion_sensitivity).to_bytes(4, "little")
            + int(2).to_bytes(2, "little")
            + int(static_sensitivity).to_bytes(4, "little")
        )
    ...
    def set_resolution(self, resolution):
        ...
        return self._command(
            COMMAND_SET_RESOLUTION, code[resolution].to_bytes(2, "little")
        )
